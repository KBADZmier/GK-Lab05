...
class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
    ...
    private void draw(GL2 gl2) {
        ...
        // range -5 to 5.)
        if(objectNumber==1){       //wybor obiektu
            corkscrew(11,gl2);
        }
        if(objectNumber==2){
            pyramid(gl2);
    ...
    private void singleTriangleDraw(float j, float k, float n, GL2 gl2)
    {
        gl2.glBegin(GL_TRIANGLE_FAN);

        gl2.glVertex3d(	(float)(Math.cos(j * 2 * Math.PI / n)),
                (float)(Math.sin(j * 2 * Math.PI / n)), 2.25);
        gl2.glVertex3d(	(float)(Math.cos(k * 2 * Math.PI / n)),
                (float)(Math.sin(k * 2 * Math.PI / n)), 2.25);
    ...
    private void pyramid(GL2 gl2)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            singleTriangleDraw(i - 1, i, n, gl2);
        }
